...
def iqCalc(atype, pos, qrange, ffactor1, ffactor2):
    ...
    for i in range(natoms):
        ...
        if a1 < 11:
            ...
            I1 += fi*fi
            ### ignore type A particles as solvent
            continue
        else:
            fi = ffactor2[a1]
            I2 += fi*fi
    ...
    #I1 /= I1[-1]
    return I2 ### ignore type A particles as solvent
    #return I1, I2
...
class scatterer_generator:
    def __init__(self,
        ...
        if path.isfile('current_sse.txt'):
            self.best_fit = np.genfromtxt('current_fit.txt')
        else:
            self.best_fit = 1e10
    ...
    def calculateScattering(self,qrange,params,output_dir,n_cores=1):
        ...
        one = pool.starmap(produceStructure, 
                zip(params,range(len(params))))
        pool.close()
        pool.join()
        # now run LAMMPS to create close-packed strucutres
        f1 = open(output_dir+'run.sh','w')
        f1.write('d={}\nfor f in $( seq 0 $1 ); do\n  lmp -in $d/lammps.relax$f > log.lammps &\n  echo "made" > "$d/done$f.txt"\ndone\nwait'.format(output_dir))
        f1.close()
        ...
        pool = mp.Pool(n_cores)
            one = pool.starmap(doScattering, zip(
    ...
    def postProcess(self,model):
        ...
        if np.min(model.fit) < self.best_fit:
            ...
            val = np.where(model.fit == self.best_fit)[0]
            pos = self.pos[val][0]
            atype = self.atype[val][0]
    ...
    def fitness(self, pop, generation, output_dir, atype, pos, iq, params, metric='chi2'):
        cs = 10
        ...
        if generation == 0:
            ...
            best_param = params[elitei].flatten()
